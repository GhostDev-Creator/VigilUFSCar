...
fetch('https://raw.githubusercontent.com/GhostDev-Creator/Dados/refs/heads/main/agua.json')
  ...
  .then(data => {
    ...
    document.getElementById('turbidezNow').textContent = `${last.turbidity ?? '--'} NTU`;
    document.getElementById('corNow').textContent = `${last.color ?? '--'} uH`;

    document.getElementById('phPercent').textContent = `${last.ph ?? '--'}`;
    document.getElementById('turbidezPercent').textContent = `${last.turbidity ?? '--'} NTU`;
    document.getElementById('corPercent').textContent = `${last.color ?? '--'} uH`;

    renderOrders(allOrders, maxInitialRows);

    data.sort((a, b) => parseDate(a.date) - parseDate(b.date));
    const labels = data.map(d => d.date);
    const st = data.map(d => d.st ?? null);
    const stv = data.map(d => d.stv ?? null);
    const stf = data.map(d => d.stf ?? null);
